...
const path = 'src/pages/KatalogUmumPage.tsx';
...
const checkoutUI = `
                      ...
                      >
                        <CheckCircle className="w-6 h-6"/> Kirim Pesanan Sekarang
                      </button>
`;

content = content.replace(
  /<div className="flex items-center justify-between mb-4 mt-4">[\s\S]*?<CheckCircle className="w-6 h-6"\/> Kirim Pesanan Sekarang\n                      <\/button>/,
  checkoutUI
);
...
console.log('Successfully updated KatalogUmumPage.tsx');
