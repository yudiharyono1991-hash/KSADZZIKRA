...
export default function MiniJadwalShalat({ isDarkMode }: { isDarkMode: boolean }) {
  ...
  return (
    <div 
      ...
      className={`p-3 sm:p-4 rounded-2xl border shadow-sm flex flex-col items-center cursor-pointer transition-all hover:-translate-y-1 hover:shadow-lg ${isDarkMode ? 'bg-slate-800/80 backdrop-blur-md border-slate-700 hover:bg-slate-800' : 'bg-white/60 backdrop-blur-md border-green-100 hover:bg-white'}`}
    >
      ...
      {loading ? (
        ...
        <div className="w-full grid grid-cols-2 gap-1 sm:gap-2 text-[10px] sm:text-xs">
          <div className={`flex justify-between p-1 sm:p-1.5 rounded bg-opacity-50 ${isDarkMode ? 'bg-slate-700 text-slate-300' : 'bg-slate-50 text-slate-600'}`}>
          ...
          <div className={`col-span-2 flex justify-between p-1 sm:p-1.5 rounded bg-opacity-50 ${isDarkMode ? 'bg-slate-700 text-slate-300' : 'bg-slate-50 text-slate-600'}`}>
