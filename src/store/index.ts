...
import { Product, CartItem, Transaction, AuditLog, ZakatCalculation, ZakatDistribution, CurrentUser, Expense, ClosingRecord, UserRole, UserAccount, PurchaseOrder, JournalEntry, JournalSourceType, Branch, Customer, Supplier, Promo, Attendance, StoreSettings, StockMovement } from '../types';
...
const getSavedUsers = (): UserAccount[] => {
  ...
  return [
    { id: 'usr_1', name: 'Kasir Asy', username: 'asy.23.kk', password: 'kasir123!', role: 'CASHIER', createdAt: new Date().toISOString(), isActive: true, isApproved: true },
    { id: 'usr_2', name: 'Superadmin BA', username: 'superadmin.23kk', password: 'admin123!', role: 'ADMIN', createdAt: new Date().toISOString(), isActive: true, isApproved: true },
    { id: 'usr_3', name: 'Owner BA', username: 'owner.23kk', password: 'owner123!', role: 'OWNER', createdAt: new Date().toISOString(), isActive: true, isApproved: true }
  ];
};
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  updateSettings: (updates) => {
    ...
    localStorage.setItem('ba_settings', JSON.stringify(updated));
  ...
  addStockMovement: (movement) => {
    ...
    localStorage.setItem('ba_stock_movements', JSON.stringify(updated));
  },
  ...
  voidTransaction: (txId, reason) => {
    ...
    localStorage.setItem('ba_transactions', JSON.stringify(updatedTransactions));
    localStorage.setItem('ba_products', JSON.stringify(updatedProducts));
    localStorage.setItem('ba_journal_entries', JSON.stringify(updatedJournals));
  ...
  addBranch: (branchData) => {
    ...
    localStorage.setItem('ba_branches', JSON.stringify(updated));
  ...
  addCustomer: (customerData) => {
    ...
    localStorage.setItem('ba_customers', JSON.stringify(updated));
  ...
  addSupplier: (supplierData) => {
    ...
    localStorage.setItem('ba_suppliers', JSON.stringify(updated));
  ...
  addPromo: (promoData) => {
    ...
    set({ promos: updated });
    localStorage.setItem('ba_promos', JSON.stringify(updated));
  ...
  clockIn: (userId, userName) => {
    ...
    localStorage.setItem('ba_attendances', JSON.stringify(updated));
  ...
  login: (username, password) => {
    const { users } = get();
    // Cek apakah user ada dengan username & password yang benar
    const foundUser = users.find(u => u.username === username && u.password === password && u.isActive);

    if (!foundUser) return 'INVALID'; // Username/password salah

    // Cek apakah akun sudah disetujui
    if (!foundUser.isApproved) return 'PENDING'; // Masih menunggu approval

    const authUser: CurrentUser = { name: foundUser.name, username: foundUser.username, role: foundUser.role, branchId: foundUser.branchId };
  ...
  logout: () => {
    ...
    set({ currentUser: null, cart: [], activeBranchId: '' });
  },
  ...
  checkout: (options) => {
    ...
    localStorage.setItem('ba_products', JSON.stringify(updatedProducts));
    localStorage.setItem('ba_transactions', JSON.stringify([newTx, ...get().transactions]));
  ...
  addProduct: (newProd) => {
    ...
    localStorage.setItem('ba_products', JSON.stringify(updated));
  ...
  adjustStock: (productId, amount) => {
    ...
    localStorage.setItem('ba_products', JSON.stringify(updatedList));
  ...
  addZakatRecord: (record) => {
    ...
    localStorage.setItem('ba_zakat_records', JSON.stringify(updated));
    get().addLog(
      'ZAKAT_CALCULATION',
      'ZAKAT',
  ...
  addZakatDistribution: (dist) => {
    ...
    localStorage.setItem('ba_zakat_distributions', JSON.stringify(updated));
    get().addLog(
      'ZAKAT_DISTRIBUTION',
      'ZAKAT',
  ...
  addPurchaseOrder: (poData) => {
    ...
    localStorage.setItem('ba_purchase_orders', JSON.stringify(updated));
  ...
  addJournalEntry: (entryData) => {
    ...
    localStorage.setItem('ba_journal_entries', JSON.stringify(updated));
  ...
  addExpense: (expenseData) => {
    ...
    localStorage.setItem('ba_expenses', JSON.stringify(updated));
  ...
  addClosing: (closingData) => {
    ...
    localStorage.setItem('ba_closings', JSON.stringify(updated));
  ...
  addLog: (action, category, details) => {
    ...
    localStorage.setItem('ba_audit_logs', JSON.stringify(updated));
        ...
        Promise.all([
          supabaseService.getProducts().then(remoteProds => {
            if (remoteProds && remoteProds.length > 0) {
              const productsMap = remoteProds.map(p => ({
                id: p.id,
                sku: p.sku,
                name: p.name,
                category: p.category,
