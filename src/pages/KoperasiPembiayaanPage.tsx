    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Kelola data pembiayaan anggota dengan berbagai pilihan akad syariah.</p>
        </div>
        <button 
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <h2 className="font-bold text-gray-800 mb-4 border-b pb-2">Form Pengajuan Pembiayaan</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Pilihan Akad</label>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Nominal Pokok (Rp)</label>
            ...
            {formData.akad !== 'Qardh' && (
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">Margin / Ujrah (Rp)</label>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Tenor (Bulan)</label>
            ...
            <tbody className="divide-y divide-gray-100">
              {data.length === 0 ? (
                ...
                <tr key={item.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="p-4 font-medium text-gray-800">{item.anggotaName}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-center">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${item.status === 'Aktif' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                      {item.status}
                    </span>
                  </td>
                  <td className="p-4 text-center">
