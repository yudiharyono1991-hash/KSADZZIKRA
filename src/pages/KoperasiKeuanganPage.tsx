    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Ringkasan kesehatan finansial khusus untuk unit simpan pinjam / pembiayaan syariah.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-green-100 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10"><Landmark className="w-16 h-16 text-green-900"/></div>
          <p className="text-sm font-bold text-gray-500 dark:text-slate-400 mb-1">Total Piutang Berjalan</p>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-blue-100 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10"><TrendingUp className="w-16 h-16 text-blue-900"/></div>
          <p className="text-sm font-bold text-gray-500 dark:text-slate-400 mb-1">Total Saldo Pokok Disalurkan</p>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-amber-100 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10"><TrendingDown className="w-16 h-16 text-amber-900"/></div>
          <p className="text-sm font-bold text-gray-500 dark:text-slate-400 mb-1">Proyeksi Pendapatan Margin/Ujrah</p>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
        <div className="p-5 border-b border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-800">
          <h2 className="font-bold text-gray-800 dark:text-slate-200 text-lg">Rincian Piutang Anggota</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-white dark:bg-slate-900 text-gray-600 dark:text-slate-400 border-b">
              <tr>
              ...
              {pembiayaan.length === 0 ? (
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50 dark:bg-slate-800/50">
                    <td className="p-4 font-bold text-gray-800 dark:text-slate-200">{item.anggotaName}</td>
                    <td className="p-4">
                      <span className="px-2 py-1 text-[10px] font-bold uppercase rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300">
                        {item.akad}
                      </span>
                    </td>
                    <td className="p-4 text-right font-medium text-gray-600 dark:text-slate-400">Rp {item.nominal.toLocaleString('id-ID')}</td>
                    <td className="p-4 text-right font-medium text-gray-600 dark:text-slate-400">Rp {item.margin.toLocaleString('id-ID')}</td>
