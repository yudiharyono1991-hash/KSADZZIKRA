            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map(p => (
                <tr key={p.id} className="hover:bg-gray-50 dark:bg-slate-800">
                  ...
                  <td className="px-6 py-4 text-gray-600 dark:text-slate-400">Rp {p.minPurchase.toLocaleString('id-ID')}</td>
                  <td className="px-6 py-4 text-center">
                    {p.isActive ? (
                      <span className="px-2 py-1 bg-green-100 text-green-800 font-bold rounded-full text-xs">Aktif</span>
                    ) : (
                      <span className="px-2 py-1 bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400 font-bold rounded-full text-xs">Nonaktif</span>
                    )}
                  </td>
                  <td className="px-6 py-4 text-center space-x-2">
                    <button onClick={() => handleEdit(p)} className="p-1.5 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:bg-slate-800 rounded-lg" title="Edit">
