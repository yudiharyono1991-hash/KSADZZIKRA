...
export default function KatalogUmumPage() {
  ...
  // All available products (in stock only for physical, ignore stock for PPOB)
  const availableProducts = useMemo(() => products.filter(p => p.isPPOB || p.stock > 0), [products]);
  ...
  const filteredProducts = useMemo(() => {
    return availableProducts.filter(p => {
      // Filter by Tab (CATALOG = Physical, PPOB = Digital)
      if (activeTab === 'CATALOG' && p.isPPOB) return false;
      if (activeTab === 'PPOB' && !p.isPPOB) return false;
  ...
  }, [availableProducts, selectedCategory, searchQuery, activeTab]);
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 dark:bg-slate-800 flex flex-col">

      {/* ── STICKY HEADER ──────────────────────────────────────── */}
      <header className="bg-gradient-to-r from-green-700 to-green-600 text-white shadow-lg sticky top-0 z-30">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/')} className="p-1.5 hover:bg-white dark:bg-slate-900/20 rounded-lg transition-colors">
          ...
          {/* Cart Indicator & Dark Mode Toggle */}
          <div className="flex items-center gap-2">
            <button onClick={toggleDarkMode} className="p-2.5 bg-white dark:bg-slate-900/20 hover:bg-white dark:bg-slate-900/30 rounded-xl transition-colors text-green-700 dark:text-amber-300" title="Ubah Tema Cahaya">
              {isDarkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
            </button>
            <button
              onClick={() => setActiveTab('CART')}
              className="relative p-2.5 bg-white dark:bg-slate-900/20 hover:bg-white dark:bg-slate-900/30 rounded-xl transition-colors text-green-700 dark:text-white"
            >
        ...
        <div className="flex border-t border-green-600">
          {([['CATALOG', '🛍️ Barang'], ['PPOB', '📱 Layanan'], ['CART', '🛒 Keranjang'], ['GUIDE', '📖 Panduan']] as const).map(([tab, label]) => (
            <button
              key={tab}
      ...
      <main className="flex-1 overflow-auto">
          ...
          <div className="bg-amber-500 text-white px-4 py-2.5 flex items-center gap-2 text-xs font-medium">
            ...
            <span>
              {!settings?.operationalHours?.isOpen 
                ? settings?.operationalHours?.closedMessage || 'Toko sedang tutup.'
                : `Di luar jam operasional (${settings?.operationalHours?.openTime || '07:00'}-${settings?.operationalHours?.closeTime || '21:00'}). Pesanan akan diproses jam kerja berikutnya.`}
            </span>
          </div>
        )}
        ...
        {(activeTab === 'CATALOG' || activeTab === 'PPOB') && (
          <div className="pb-6">
            ...
            {/* Search Bar */}
            <div className="px-4 pt-4 pb-3 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 sticky top-0 z-10 space-y-3">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <input
                  type="search"
                  placeholder={`Cari dari ${availableProducts.length.toLocaleString('id-ID')} produk...`}
                  className="w-full pl-9 pr-10 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-green-500 outline-none focus:bg-white dark:bg-slate-900 transition-colors"
                  value={searchQuery}
                  onChange={e => setSearchQuery(e.target.value)}
                />
                {searchQuery && (
                  <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:text-slate-400">
                    <X className="w-4 h-4" />
                  </button>
                )}
              </div>
            ...
            {/* Stats Bar */}
            <div className="px-4 py-2 bg-slate-50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Menampilkan <span className="font-bold text-slate-700 dark:text-slate-300">{filteredProducts.length.toLocaleString('id-ID')}</span> produk
            ...
            {isDataSyncing && availableProducts.length === 0 && (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm">Memuat katalog produk...</p>
              </div>
            )}
          ...
          <div className="p-4 pb-8 max-w-3xl mx-auto">
            {customerCart.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-700 dark:text-slate-300 mb-1">Keranjang Kosong</h3>
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Tambahkan produk dari katalog belanja kami.</p>
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <h2 className="font-bold text-slate-800 dark:text-slate-200 text-lg">Pesanan Anda ({cartCount} item)</h2>
                  {customerCart.map(item => (
                    <div key={item.product.id} className="bg-white dark:bg-slate-900 p-3 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center gap-3">
                      <div className="w-14 h-14 bg-slate-100 dark:bg-slate-800 rounded-lg overflow-hidden flex-shrink-0">
                ...
                <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex justify-between items-center">
                  <span className="font-bold text-slate-700 dark:text-slate-300">Total Belanja</span>
                  ...
                  <form onSubmit={handleCheckout} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input type="text" required value={customerName} onChange={e => setCustomerName(e.target.value)}
                        className="w-full border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm focus:ring-2 focus:ring-green-500 outline-none" placeholder="Masukkan nama lengkap" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input type="text" value={checkoutNotes} onChange={e => setCheckoutNotes(e.target.value)}
                        className="w-full border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm focus:ring-2 focus:ring-green-500 outline-none" placeholder="Misal: minta tas/kantong ekstra" />
                    </div>

                    {/* Payment Method */}
                    <div>
                      <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-2">Metode Pembayaran *</label>
                        ...
                        {(['COD', 'TRANSFER', 'EWALLET', 'QRIS'] as const).map(method => {
                          ...
                          return (
                            <button key={method} type="button" onClick={() => setPaymentMethod(method)}
                              className={`p-3 rounded-xl border-2 text-xs font-bold text-center transition-all ${paymentMethod === method ? colors[method] : 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:bg-slate-800'}`}>
                              {labels[method]}
                            </button>
                          );
                        })}
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 text-xs space-y-3">
                      <p className="font-bold text-slate-800 dark:text-slate-200 border-b pb-2">💳 Instruksi Pembayaran {selectedBranchData ? `(${selectedBranchData.name})` : ''}</p>
                      {paymentMethod === 'COD' && (
                        <p className="text-slate-600 dark:text-slate-400">Siapkan uang tunai sebesar <b className="text-green-700">Rp {cartTotal.toLocaleString('id-ID')}</b> saat kurir tiba.</p>
                      )}
                      {paymentMethod === 'TRANSFER' && (
                        <div className="space-y-2">
                          <p className="text-slate-600 dark:text-slate-400">Transfer <b className="text-green-700">Rp {cartTotal.toLocaleString('id-ID')}</b> ke:</p>
                          {((selectedBranchData?.paymentMethods?.bankTransfer?.length) ? selectedBranchData.paymentMethods.bankTransfer : (settings.paymentMethods?.bankTransfer?.length ? settings.paymentMethods.bankTransfer : [{ bankName: 'BSI (Bank Syariah Indonesia)', accountNumber: '7182938495', accountName: 'KSA Mart Syariah' }])).map((b, i) => (
                            <div key={i} className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-blue-200">
                              <p className="font-bold text-blue-800">{b.bankName}</p>
                              <p className="font-mono text-base mt-0.5">{b.accountNumber}</p>
                              <p className="text-slate-500 dark:text-slate-400 text-[11px]">a.n. {b.accountName}</p>
                            </div>
                          ))}
                        </div>
                      )}
                      {paymentMethod === 'EWALLET' && (
                        <div className="space-y-2">
                          <p className="text-slate-600 dark:text-slate-400">Transfer <b className="text-green-700">Rp {cartTotal.toLocaleString('id-ID')}</b> via e-wallet:</p>
                          {((selectedBranchData?.paymentMethods?.ewallet?.length) ? selectedBranchData.paymentMethods.ewallet : (settings.paymentMethods?.ewallet?.length ? settings.paymentMethods.ewallet : [{ provider: 'DANA / OVO', number: 'Belum diatur', accountName: '' }])).map((w, i) => (
                            <div key={i} className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-purple-200">
                              <p className="font-bold text-purple-800">{w.provider}</p>
                              <p className="font-mono text-base mt-0.5">{w.number}</p>
                              {w.accountName && <p className="text-slate-500 dark:text-slate-400 text-[11px]">a.n. {w.accountName}</p>}
                            </div>
                          ))}
                        </div>
                      )}
                      {paymentMethod === 'QRIS' && (
                        <div className="flex flex-col items-center gap-2">
                          <p className="text-slate-600 dark:text-slate-400 text-center">Scan QRIS dengan nominal <b className="text-green-700">Rp {cartTotal.toLocaleString('id-ID')}</b>:</p>
                          {effectiveQrisUrl
                            ? (
                              <div className="flex flex-col items-center">
                                <img 
                                  ...
                                  alt="QRIS" 
                                  className="w-40 h-auto rounded-xl border-2 border-fuchsia-200 p-1 bg-white dark:bg-slate-900 shadow-sm cursor-pointer hover:opacity-90 transition-opacity" 
          ...
          <div className="p-4 pb-8 max-w-2xl mx-auto space-y-4">
            <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl">
              ...
              <ul className="text-amber-800 text-sm leading-relaxed space-y-1.5 list-disc list-inside">
                <li><strong>Jam operasional:</strong> 07.00 - 21.00 WIB setiap hari</li>
            ...
            {[
              ...
              <div key={step.num} className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm flex gap-4">
      ...
      {(activeTab === 'CATALOG' || activeTab === 'PPOB') && cartCount > 0 && (
      ...
      {settings.ownerWhatsapp && (
        <a
          ...
          className={`fixed z-50 flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-4 rounded-full shadow-2xl transition-all duration-300 hover:scale-105 active:scale-95 group ${(activeTab === 'CATALOG' || activeTab === 'PPOB') && cartCount > 0 ? 'bottom-20' : 'bottom-6'} right-4`}
        >
      ...
      {showQrisZoom && effectiveQrisUrl && (
        <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4" onClick={() => setShowQrisZoom(false)}>
          <div className="bg-white dark:bg-slate-900 p-4 rounded-3xl max-w-sm w-full shadow-2xl relative" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-center font-bold text-slate-800 dark:text-slate-200 mb-1">Scan QRIS KSA Mart</h3>
            {selectedBranchData && (
