      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-slate-500 mt-1">Hierarki Standar Koperasi Syariah KSA Mart</p>
          </div>
        </div>
      </div>

      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 md:p-12 w-full mx-auto overflow-x-auto flex flex-col items-center min-w-[700px]">
        ...
        <div className="flex flex-col items-center w-full">
          <div className="bg-rose-50 border-2 border-rose-400 p-6 rounded-2xl w-full max-w-lg text-center shadow-lg relative z-10">
            ...
            <h2 className="text-2xl font-black text-slate-800 mt-1">R.A.T & ANGGOTA KOPERASI</h2>
            <div className="flex justify-center gap-4 mt-3">
              <span className="bg-white border border-rose-200 text-rose-700 text-xs font-bold px-3 py-1 rounded-full">{totalPelanggan} Anggota Terdaftar</span>
            </div>
          </div>
        ...
        <div className="flex justify-center gap-16 w-[700px] mt-8 relative z-10">
          ...
          <div className="flex flex-col items-center flex-1">
            <div className="bg-purple-50 border-2 border-purple-400 p-6 rounded-2xl w-full text-center shadow-lg relative z-10">
              ...
              <h2 className="text-lg font-black text-slate-800 mt-1">PENGAWAS & DPS</h2>
              <div className="mt-4 flex flex-col gap-2">
                {dewanPengawasDps.length > 0 ? dewanPengawasDps.map(p => (
                  <div key={p.id} className="bg-white p-2 rounded-lg border border-purple-200 shadow-sm flex flex-col">
                    <span className="font-bold text-slate-700 text-sm">{p.name}</span>
                    <span className="text-[9px] font-bold text-purple-600 uppercase">{p.jobTitle}</span>
                  </div>
                )) : (
                  <div className="bg-white p-2 rounded-lg border border-purple-200 text-xs text-slate-400 italic shadow-sm">Belum ada ditunjuk</div>
                )}
              </div>
            </div>
          ...
          <div className="flex flex-col items-center flex-1">
            <div className="bg-amber-50 border-2 border-amber-400 p-6 rounded-2xl w-full text-center shadow-lg relative z-10">
              ...
              <h2 className="text-lg font-black text-slate-800 mt-1">PENGURUS</h2>
              <div className="mt-4 flex flex-col gap-2">
                <div className="bg-amber-100 p-2 rounded-lg border-2 border-amber-300 shadow-sm flex flex-col">
                    <span className="font-black text-slate-800 text-sm">{owner.name}</span>
                    <span className="text-[9px] font-bold text-amber-700 uppercase">{owner.jobTitle || 'Ketua Koperasi'}</span>
                </div>
                {jajaranPengurus.map(p => (
                  <div key={p.id} className="bg-white p-2 rounded-lg border border-amber-200 shadow-sm flex flex-col">
                    <span className="font-bold text-slate-700 text-sm">{p.name}</span>
        ...
        <div className="flex flex-col items-center w-full mt-4 translate-x-[183px]">
          <div className="bg-indigo-50 border-2 border-indigo-400 p-4 rounded-2xl w-[318px] text-center shadow-lg relative z-10">
            ...
            <h2 className="text-md font-black text-slate-800">MANAGER</h2>
            <div className="mt-2 flex flex-col gap-1">
              {managers.length > 0 ? managers.map(m => (
                <div key={m.id} className="bg-white p-1.5 rounded-lg border border-indigo-200 font-bold text-slate-700 text-sm shadow-sm">{m.name}</div>
              )) : (
                <div className="bg-white p-1.5 rounded-lg border border-indigo-200 text-xs text-slate-400 italic shadow-sm">Belum ditunjuk</div>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-col items-center w-full mt-3 translate-x-[183px]">
          <div className="bg-blue-50 border-2 border-blue-400 p-4 rounded-2xl w-[318px] text-center shadow-lg relative z-10">
            <h2 className="text-md font-black text-slate-800">ADMINISTRATOR</h2>
            <div className="mt-2 flex flex-wrap justify-center gap-1">
              {admins.length > 0 ? admins.map(a => (
                <div key={a.id} className="bg-white px-3 py-1 rounded-lg border border-blue-200 font-bold text-slate-700 text-xs shadow-sm">{a.name}</div>
              )) : (
                <div className="bg-white p-1.5 rounded-lg border border-blue-200 text-xs text-slate-400 italic shadow-sm w-full">Belum ditunjuk</div>
              )}
            </div>
          </div>
          <div className="h-8 w-1 bg-slate-300 relative"><ArrowDown className="absolute -bottom-3 left-1/2 -translate-x-1/2 w-5 h-5 text-slate-400" /></div>
        ...
        <div className="flex flex-col items-center w-full mt-3 translate-x-[183px]">
          <div className="bg-emerald-50 border-2 border-emerald-400 p-4 rounded-2xl w-[318px] text-center shadow-lg relative z-10">
            <h2 className="text-md font-black text-slate-800">TIM KASIR</h2>
            <div className="mt-2 flex flex-wrap justify-center gap-1">
              {cashiers.length > 0 ? cashiers.map(c => (
                <div key={c.id} className="bg-white px-3 py-1 rounded-lg border border-emerald-200 font-bold text-slate-700 text-xs shadow-sm">{c.name}</div>
              )) : (
                <div className="bg-white p-1.5 rounded-lg border border-emerald-200 text-xs text-slate-400 italic shadow-sm w-full">Belum ditunjuk</div>
              )}
            </div>
          </div>
          <div className="h-8 w-1 bg-slate-300 relative"><ArrowDown className="absolute -bottom-3 left-1/2 -translate-x-1/2 w-5 h-5 text-slate-400" /></div>
        ...
        <div className="flex flex-col items-center w-full mt-3 translate-x-[183px]">
          <div className="bg-slate-50 border-2 border-slate-300 p-4 rounded-2xl w-[318px] text-center shadow-lg relative z-10">
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Konsumen</p>
            <h2 className="text-md font-black text-slate-800">PELANGGAN UMUM</h2>
            <div className="mt-2">
              <div className="bg-white p-2 rounded-lg border border-slate-200 font-bold text-slate-700 shadow-sm flex items-center justify-between mt-1 text-xs">
